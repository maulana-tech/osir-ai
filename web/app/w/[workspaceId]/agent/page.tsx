...
import { markHandled, sendCommand, setAutonomy } from "@/app/actions";
import { Empty, PageHeader, RunRow, Section, timeAgo } from "@/app/components/ui";
...
export default async function Autopilot({ params }: { params: Promise<{ workspaceId: string }> }) {
  const { workspaceId } = await params;
  const base = `/w/${workspaceId}/agent`;
  const [policy, decisions, runs] = await Promise.all([
    api.policy(workspaceId),
    api.decisions(workspaceId, true),
    api.runs(workspaceId, 8),
  ]);
  ...
  return (
    <div>
      <PageHeader title="Autopilot">
        <Link href={`${base}/runs`} className="text-sm" style={{ color: "var(--muted)" }}>
          All runs →
        </Link>
      </PageHeader>
          ...
          <Section title="Ask Osir" hint={policy.agentcore_configured ? "runs on AgentCore" : "runs on the local worker"}>
            ...
            {running > 0 && (
              <p className="mt-3 text-xs" style={{ color: "var(--muted)" }}>
                {running} run{running > 1 ? "s" : ""} in progress. This page refreshes every 30 seconds.
              </p>
            )}
          </Section>
