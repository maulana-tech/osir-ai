...
# Database
DATABASES = {
    "default": env.db("DATABASE_URL", default="postgres://postgres:postgres@localhost:5432/osir_ai"),
}
...
#   2. ``STORAGE_QUOTA_TIERS[plan_slug]`` (no plan source wired today).
...
STORAGE_QUOTA_TIERS = {
    # plan_slug → bytes, for a future billing integration.
    "hobby": 5 * 1024**3,  # 5 GB
...
OAUTH2_PROVIDER = {
    "SCOPES": {"mcp": "Call Osir AI MCP tools on your behalf"},
    "DEFAULT_SCOPES": ["mcp"],
    "PKCE_REQUIRED": True,
...
# Osir AI autopilot (agent/). When AGENT_RUNTIME_ARN is set, console commands
# are handed to Amazon Bedrock AgentCore Runtime; otherwise they wait for a
# local runner (``python agent/run_local.py worker``).
AGENT_RUNTIME_ARN = env("AGENT_RUNTIME_ARN", default="")
AGENT_RUNTIME_REGION = env("AGENT_RUNTIME_REGION", default="us-west-2")

# Composio: fallback OAuth broker for orgs without their own developer apps.
# Create one Composio-managed auth config per toolkit in the Composio dashboard
# and map it here; platforms without an entry keep requiring own credentials.
COMPOSIO_API_KEY = env("COMPOSIO_API_KEY", default="")
COMPOSIO_AUTH_CONFIGS = {
    platform: value
    for platform, value in {
        "linkedin_personal": env("COMPOSIO_AUTH_CONFIG_LINKEDIN", default=""),
        "linkedin_company": env("COMPOSIO_AUTH_CONFIG_LINKEDIN", default=""),
        "facebook": env("COMPOSIO_AUTH_CONFIG_FACEBOOK", default=""),
        "instagram": env("COMPOSIO_AUTH_CONFIG_FACEBOOK", default=""),
        "instagram_login": env("COMPOSIO_AUTH_CONFIG_INSTAGRAM", default=""),
        "youtube": env("COMPOSIO_AUTH_CONFIG_YOUTUBE", default=""),
        "pinterest": env("COMPOSIO_AUTH_CONFIG_PINTEREST", default=""),
    }.items()
    if value
}
