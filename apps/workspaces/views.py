...
def approvals_settings(request, workspace_id):
    ...
    if request.method == "POST":
        ...
        autonomy = request.POST.get("agent_autonomy", workspace.agent_autonomy)
        if autonomy not in Workspace.AgentAutonomy.values:
            messages.error(request, "Invalid agent autonomy level.")
            return redirect("workspaces:approvals_settings", workspace_id=workspace.id)

        workspace.approval_workflow_mode = mode
        workspace.agent_autonomy = autonomy
        workspace.save(update_fields=["approval_workflow_mode", "agent_autonomy", "updated_at"])
    ...
    return render(
        ...
        "workspaces/approvals_settings.html",
        {
            ...
            "autonomy_levels": Workspace.AgentAutonomy,
        },
    )
