...
def _resolve_unused_idea(api_key, args: dict):
    """Resolve ``idea_id`` to an Idea in the key's workspace that has no post yet."""
    idea_id = _parse_uuid(args["idea_id"], "idea_id")
    try:
        idea = Idea.objects.for_workspace(api_key.workspace_id).get(id=idea_id)
    except Idea.DoesNotExist as exc:
        raise JsonRpcError(INVALID_PARAMS, "idea_id not found in this workspace") from exc
    if idea.post_id:
        raise JsonRpcError(INVALID_PARAMS, "That idea already has a post")
    return idea
...
def _create_draft(args: dict, context: dict[str, Any]) -> dict:
    ...
    idea = _resolve_unused_idea(api_key, args) if args.get("idea_id") else None
    ...
    if idea is not None:
        idea.post = post
        idea.status = Idea.Status.IN_PROGRESS
        ...
        input_schema={
            ...
            "properties": {
                ...
                "idea_id": {
                    "type": "string",
                    "format": "uuid",
                    "description": "Optional Idea (from list_ideas) this draft realises; links it and marks it in progress.",
                },
            },
