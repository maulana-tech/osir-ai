...
COMPOSIO_SESSION_KEY = "social_composio"
...
def connect_platform(request, workspace_id):
    ...
    configured_platforms = _get_configured_platforms(request.org.id)
    # Platforms the org has no developer app for but Composio can broker.
    composio_platforms = composio.available_platforms() - configured_platforms
    ...
    if request.method == "GET":
        return render(
            ...
            "social_accounts/connect.html",
            {
                ...
                "configured_platforms": configured_platforms,
                "composio_platforms": composio_platforms,
            },
        )
    ...
    if platform not in configured_platforms:
        if platform in composio_platforms:
            return _start_composio_connect(request, workspace_id, platform)
        messages.error(
            request,
...
# ------------------------------------------------------------------
# Composio fallback (hosted OAuth, proxied calls)
# ------------------------------------------------------------------
...
def _start_composio_connect(request, workspace_id, platform):
    """Send the user to Composio's hosted OAuth for ``platform``."""
    from django.urls import reverse

    callback_url = request.build_absolute_uri(reverse("social_accounts:composio_callback"))
    try:
        connected_account_id, redirect_url = composio.create_link(
            platform=platform,
            # Composio's "user" is the entity that owns the connection: the org.
            user_id=str(request.org.id),
            callback_url=callback_url,
        )
    except composio.ComposioError:
        logger.exception("Composio link failed for %s", platform)
        messages.error(request, "Could not start the connection. Please try again.")
        return redirect("social_accounts:connect", workspace_id=workspace_id)

    request.session[COMPOSIO_SESSION_KEY] = {
        "workspace_id": str(workspace_id),
        "platform": platform,
...
@ratelimit(key="user", rate="20/m", block=True)
@require_GET
def composio_callback(request):
    """Composio sends the user back here once the hosted OAuth completed."""
    session_data = request.session.pop(COMPOSIO_SESSION_KEY, {})
    if not session_data or session_data.get("user_id") != str(request.user.id):
        messages.error(request, "Connection session mismatch. Please try again.")
        return redirect("dashboard")

    workspace_id = session_data["workspace_id"]
    platform = session_data["platform"]
    connected_account_id = session_data["connected_account_id"]
    if request.GET.get("connected_account_id", connected_account_id) != connected_account_id:
        messages.error(request, "Connection mismatch. Please try again.")
        return redirect("social_accounts:connect", workspace_id=workspace_id)

    from apps.members.models import WorkspaceMembership

    ws_membership = WorkspaceMembership.objects.filter(user=request.user, workspace_id=workspace_id).first()
    if not ws_membership or not ws_membership.effective_permissions.get("manage_social_accounts", False):
        raise PermissionDenied("You no longer have permission to manage social accounts.")

    try:
        if composio.connection_status(connected_account_id) != "ACTIVE":
            messages.error(request, "The connection was not completed. Please try again.")
            return redirect("social_accounts:connect", workspace_id=workspace_id)

        provider = _get_provider_for_platform(
            platform, request.org.id, **composio.proxy_credentials(connected_account_id)
        )

        if platform in _PAGE_PLATFORMS and hasattr(provider, "get_user_pages"):
            pages = provider.get_user_pages("")
            if not pages:
                messages.warning(request, "No pages were found on that account.")
                return redirect("social_accounts:list", workspace_id=workspace_id)
            request.session["oauth_page_select"] = {
                "workspace_id": workspace_id,
                "platform": platform,
                "user_tokens": {"access_token": "", "refresh_token": ""},
        ...
        messages.success(request, f"Connected {profile.name} via Composio.")
    except PermissionDenied:
        raise
    except Exception:
        logger.exception("Composio callback failed for %s", platform)
...
def select_account(request):
    ...
    composio_id = page_data.get("composio_connected_account_id", "")
    connected = []
    ...
    for page in page_data["pages"]:
        if page["id"] in selected_ids:
            ...
            # A page token from the platform (Facebook) is used directly even
            # when the login ran through Composio; without one the account
            # stays Composio-backed and every call is proxied.
            auth_source = SocialAccount.AuthSource.OWN
            if not access_token and composio_id:
                auth_source = SocialAccount.AuthSource.COMPOSIO
            ...
            _create_or_update_account(
                ...
                composio_connected_account_id=composio_id if auth_source == SocialAccount.AuthSource.COMPOSIO else "",
            )
    ...
    *,
    ...
    account, created = SocialAccount.objects.update_or_create(
        ...
        defaults={
            ...
            "auth_source": auth_source,
            "composio_connected_account_id": composio_connected_account_id or "",
