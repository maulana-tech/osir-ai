...
2. ``settings.STORAGE_QUOTA_TIERS[plan_slug]`` (no plan source wired today)
...
class StorageQuota:
    limit_bytes: int
    plan_slug: str  # Empty string unless a billing integration provides one.
...
def _resolve_plan_slug(organization) -> str:
    """Return the org's plan slug, or ``""``.

    There is no subscription source in this deployment, so every org is on
    ``STORAGE_QUOTA_DEFAULT`` unless an OrgSetting override exists. Kept as a
    hook so a billing integration can plug in without touching callers.
    """
    return ""
