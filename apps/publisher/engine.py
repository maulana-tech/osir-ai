...
def _resolve_publish_credentials(account):
    ...
    if getattr(account, "auth_source", "") == "composio" and account.composio_connected_account_id:
        from apps.credentials.composio import proxy_credentials

        credentials.update(proxy_credentials(account.composio_connected_account_id))
