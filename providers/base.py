...
class SocialProvider(ABC):
    ...
    def _request(
        ...
        *,
        ...
        # Accounts connected through Composio have no token of their own:
        # Composio holds it and injects it when the call goes through its proxy.
        composio_id = (self.credentials or {}).get("composio_connected_account_id")
        if composio_id:
            from .composio_transport import proxy_request

            return self._raise_for_status(
                proxy_request(
                    api_key=self.credentials.get("composio_api_key", ""),
                    connected_account_id=composio_id,
                    method=method,
                    url=url,
                    headers=req_headers,
                    params=params,
    ...
    def _raise_for_status(self, response: httpx.Response) -> httpx.Response:
        """Map 429 / 4xx / 5xx to provider exceptions; return the response otherwise."""
