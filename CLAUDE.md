...
- Routes: `/w/[workspaceId]/{calendar,calendar/slots,calendar/queues,inbox,analytics,approvals,agent,channels,media,compose,create,categories,import,settings}`, `/org/{settings,workspaces,calendar,members,media,api-keys}`, `/me/{account,notifications}`; `app/components/Shell.tsx` is the frame (sidebar + onboarding checklist). Still Django-rendered: login/signup/2FA/password reset, OAuth connect, client portal, onboarding tokens, invitation accept; file downloads and the video stream/filmstrip endpoints stay Django too. Retired Django page URLs are `apps.common.console.console(...)` redirects so `reverse()` in emails still works.
- Composer logic lives in `apps/composer/editor.py` (`EditorPayload` → `save()`), with ideas/feeds/unsplash/csv_import/platform_info modules beside it; the web API router is `apps/webapi/routers/composer.py`.
