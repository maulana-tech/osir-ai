...
- Routes: `/w/[workspaceId]/{calendar,inbox,analytics,approvals,agent,channels,media,settings}`, `/org/{settings,workspaces,members,media,api-keys}`, `/me/{account,notifications}`; `app/components/Shell.tsx` is the frame (sidebar + onboarding checklist). Still Django: composer, login/signup/2FA, OAuth connect, client portal, onboarding tokens, file downloads.
